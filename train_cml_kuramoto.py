...
def val_dynamics(
    ...
    for batch_idx, data in enumerate(val_loader):
        data = data.to(device)
        loss, mse = val_dynamics_learner(
            dynamics_learner, matrix, args.nodes, data, args.prediction_steps, skip_conn
        )
        loss_record.append(loss.item())
        mse_record.append(mse.item())

    print(
        "\nDynamics validation: loss: %f, MSE: %f"
        % (np.mean(loss_record), np.mean(mse_record))
    )
...
def train_gumbel(
    ...
    for step in range(1, args.reconstruct_steps + 1):
        ...
        for batch_idx, data in enumerate(train_loader):
            data = data.to(device)
            loss, _ = train_net_reconstructor(
                optimizer_network,
                gumbel_generator,
                dynamics_learner,
                args.nodes,
...
def test(
    ...
    for batch_idx, data in enumerate(test_loader):
        data = data.to(device)
        loss, mse = val_dynamics_learner(
            dynamics_learner, matrix, args.nodes, data, args.prediction_steps
        )
        loss_record.append(loss.item())
        mse_record.append(mse.item())
...
def main():
    ...
    parser.add_argument(
        "--nodes", type=int, default=3894, help="number of nodes in data"
    )
    parser.add_argument(
        "--dims",
        type=int,
    ...
    parser.add_argument(
        "--seed", type=int, default=2050, help="random seed (default: 2050)"
    )
    parser.add_argument(
        "--prediction-steps",
        type=int,
    ...
    if args.simulation_type == "kuramoto":
        train_loader, val_loader, test_loader, object_matrix = load_kuramoto_ggn(
            batch_size=args.batch
        )
    elif args.simulation_type == "cml":
        train_loader, val_loader, test_loader, object_matrix = load_cml_ggn(
            batch_size=args.batch
        )
    else:
