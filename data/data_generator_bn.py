...
def init_node(dg):
    for i in range(dg.number_of_nodes()):
        dg._node[i]["value"] = random.randint(0, 1)
...
def spread(dg, table, step=-1, ignore_attractor="mind_attractor"):
    ...
    for i in range(node_num):
        origin_val.append(dg._node[i]["value"])
    ...
    while ~fall_attractor and run < step:
        ...
        for i in range(node_num):
            ...
            for iter, val in enumerate(innodes[i]):
                b2t += dg._node[val]["value"] * math.pow(2, len(innodes[i]) - iter - 1)
            next_val.append(table[i][b2t])
        # print(next_val)
        # set value to the net
        for i in range(node_num):
            dg._node[i]["value"] = next_val[i]
        ...
        while go:
            ...
            if curnode in stack[:-1]:
                ...
                for j in stack:
                    ...
                    # cycle
                    elif (j == curnode or not before) and p_graph.node[j][
                        "fixpoint"
                    ] == False:
...
while len(has_explored) < Goal_Data_Num:
    ...
    for j in range(N_Node):
        init_state.append(dg._node[j]["value"])
