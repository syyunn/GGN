...
def constructor_evaluator(gumbel_generator, tests, obj_matrix, sz):
    ...
    for t in range(tests):
        ...
        err = torch.sum(
            torch.abs(out_matrix_c * get_offdiag(sz) - obj_matrix * get_offdiag(sz))
        )
...
def load_bn_ggn(batch_size=128, dyn_type="table"):
    ...
    raw_data = np.load('/Users/suyeol/KTRACK/GGN_bnuss/gvc.npy')
    # 预处理成分类任务常用的数据格式
    for i in range(int(info_train.shape[0] / 2)): # two-tuple window
        for j in range(info_train.shape[1]):
            if info_train[2 * i][j][0] == 0.0:
                data_x[i][j] = [1, 0]
            else:
                data_x[i][j] = [0, 1]
            if info_train[2 * i + 1][j][0] == 0.0:
                data_y[i][j] = 0
            else:
                data_y[i][j] = 1
...
def load_cml_ggn(batch_size=128):
    ...
    # data_path = "/tmp/pycharm_project_504/data/cml/trade_data_min_max_normalized.pickle"
    data_path = "/tmp/pycharm_project_910/data/cml/trade_data_min_max_normalized.pickle"

    with open(data_path, "rb") as f:
        object_matrix, train_data, val_data, test_data = pickle.load(
            f
        )  # (samples, nodes, timesteps, 1)
...
def train_batch_dyn_bn(
    ...
    for batch_idx, (data_train, data_target) in enumerate(train_data_loader):
        ...
        loss, accu = train_dyn_learner_bn(
            optimizer_dyn, dyn_learner, adj, data_train, data_target, loss_fn
        )
...
def tran_batch_dyn(
    ...
    for step in range(1, args.dynamics_steps + 1):
        ...
        for batch_idx, data in enumerate(train_loader):
            data = data.to(device)
            loss, mse = train_dynamics_learner(
                optimizer,
                dynamics_learner,
                matrix,
                data,
...
def train_batch_net(
    train_data_loader, optimizer_network, gumbel_generator, dyn_learner, loss_fn
):
    step_loss = 0
    step_accu = []
    for batch_idx, (data_train, data_target) in enumerate(train_data_loader):
        if use_cuda:
            ...
            data_target = data_target.cuda()
        loss, out_matrix = train_batch_generator(
            optimizer_network,
            gumbel_generator,
            dyn_learner,
            data_train,
...
def train_batch_dyn_cml(
    ...
    for step in range(1, args.dynamics_steps + 1):
        ...
        print(
            "\nDynamics learning step: %d, loss: %f, MSE: %f"
            % (step, np.mean(loss_record), np.mean(mse_record))
        )


# dyn trainer
def train_dyn_learner_bn(
    optimizer,
    dyn_learner,
    adj,
    data_train,
    ...
    if positional:
        pos_arr = torch.tensor(np.arange(0.0, float(num_nodes))).repeat(
            data_train.shape[0], 1
        )
        if use_cuda:
            pos_arr = pos_arr.cuda()
        pos_res = pos_enc(pos_arr)
        pos_res = torch.unsqueeze(pos_res, 2).repeat(
            1, 1, 2
        )  # (batch_size,node_num) => (batch_size,node_num,2)
...
def train_batch_generator(
    optimizer_network, gumbel_generator, dyn_learner, data_train, data_target, loss_fn
):
